...
const toTitleCase = (arr) => {
  return arr.map((str) => str[0].toUpperCase() + str.substring(1));
};
...
let biggerThanLuke = (arr) => {
  let luke = arr.find((obj) => obj.name === 'Luke Skywalker');
  return arr
    .filter((char) => Number(char.mass) > Number(luke.mass))
    .map((obj) => obj.name)
    .join(' - ');
};
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return /[http]+s:\/{2}\w+.\w+/gm.test(url);
};
